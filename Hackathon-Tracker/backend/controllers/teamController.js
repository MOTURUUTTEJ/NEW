...
const updateTeamProfile = async (req, res) => {
    try {
        ...
        if (user) {
            const updatedUser = {
                ...user,
                team_name: req.body.team_name !== undefined ? req.body.team_name : user.team_name,
                ...
                members: req.body.members !== undefined ? req.body.members : user.members,
            ...
            await dynamoService.saveActivity({
                ...
                detail: `Updated team profile details`,
                email: req.user.email,
                team_name: updatedUser.team_name,
                icon: 'Settings'
            });
...
const createHackathon = async (req, res) => {
    try {
        ...
        await dynamoService.saveActivity({
            ...
            detail: `Launched new hackathon: ${hackathon_name}`,
            email: req.user.email,
...
const createProject = async (req, res) => {
    try {
        ...
        await dynamoService.saveActivity({
            ...
            detail: `Initialized project: ${project_title}`,
            email: req.user.email,
            team_name: req.user.team_name,
            icon: 'Code2'
        });
...
const uploadReport = async (req, res) => {
    try {
        ...
        await dynamoService.saveActivity({
            ...
            detail: `Deployed artifact: ${req.file.originalname}`,
            email: req.user.email,
            team_name: req.user.team_name,
            icon: 'UploadCloud'
        });
...
// @desc    Get signed URL for artifact download/view
...
const getActivities = async (req, res) => {
    try {
        const activities = await dynamoService.getActivitiesByUser(req.user.email, 15);
        res.json(activities);
    } catch (error) {
...
// @desc    Get all admin-published global hackathons (visible to all teams)
const getGlobalHackathons = async (req, res) => {
    try {
        const hackathons = await dynamoService.getGlobalHackathons();
        res.json(hackathons || []);
    } catch (error) {
        // Non-fatal: return empty list if the GLOBAL#HACKATHONS partition doesn't exist yet
        console.warn('getGlobalHackathons warning:', error.message);
        res.json([]);
    }
};
