    ...
    useEffect(() => {
        const fetchLeaderboard = async () => {
            try {
                ...
                const { data } = await api.get('/api/team/leaderboard', config);
                setTeams(data);
            } catch (error) {
            ...
            <div className="flex items-center justify-between mb-2">
                <div>
                    <h2 className="text-2xl font-black text-slate-800 flex items-center gap-2">
                        <Trophy className="text-[#3b82f6]" size={28} aria-hidden="true" /> RANKINGS
                    </h2>
            ...
            <div className="grid grid-cols-1 gap-4">
                        ...
                        <div className="flex items-center gap-5">
                            ...
                            <div>
                                <h3 className="font-extrabold text-slate-800 flex items-center gap-2">
                                    {project.team_id?.team_name || 'Unknown Team'}
                                    {index === 0 && <Award size={14} className="text-amber-500" aria-hidden="true" />}
                                </h3>
                                <div className="flex items-center gap-3 mt-1">
                                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1">
                                        <Shield size={10} aria-hidden="true" /> {project.team_id?.college || 'Independent'}
                                    </span>
                        ...
                        <div className="text-right">
                            {project.isRated ? (
                                ...
                                <div className="flex items-center gap-2 px-3 py-1.5 bg-slate-100 rounded-xl border border-slate-200">
                                    <Clock size={12} className="text-slate-400 animate-pulse" aria-hidden="true" />
                                    <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest italic">Pending Review</span>
                                </div>
                            )}
                        </div>
                    </motion.div>
                ...
                {teams.length === 0 && (
                    <div className="p-20 text-center bg-slate-50 rounded-3xl border border-dashed border-slate-200">
                        <Trophy className="mx-auto text-slate-200 mb-4" size={48} aria-hidden="true" />
