...
const Login = ({ isAdmin = false }) => {
    ...
    return (
        <div className="flex flex-col justify-center min-h-screen py-12 sm:px-6 lg:px-8 relative overflow-hidden bg-transparent">
            ...
            <Link to="/welcome" className="fixed top-6 left-6 z-50 flex items-center gap-2 px-5 py-2.5 bg-white/40 backdrop-blur-xl border border-white/60 rounded-full text-slate-500 hover:text-[#3b82f6] hover:bg-white/70 transition-all font-bold uppercase tracking-widest text-[10px] shadow-sm hover:shadow-md">
                &larr; Return Hub
            </Link>
                ...
                <div className="card px-4 py-8 sm:px-10">
                    <form className="space-y-6" onSubmit={handleSubmit}>
                        ...
                        <div className="space-y-1">
                            ...
                            <input
                                id="email" type="email" name="email" required className="input-field"
                        ...
                        <div className="space-y-1">
                            ...
                            <div className="relative">
                                <input
                                    id="password" type={showPassword ? 'text' : 'password'} name="password" required className="input-field pr-12"
                                    value={password} onChange={(e) => setPassword(e.target.value)}
                                    placeholder="••••••••"
                                    autoComplete="current-password"
                                />
                                <button type="button" onClick={() => setShowPassword(!showPassword)} aria-label={showPassword ? 'Hide password' : 'Show password'} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-[#3b82f6] transition-colors">
