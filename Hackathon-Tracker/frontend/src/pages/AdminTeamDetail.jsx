...
import { ArrowLeft, User, Code2, Download, FileText, Calendar, Database, Server, Clock, Star, MessageSquare, CheckCircle2, ShieldCheck, Mail, Building, Trash2, Flag, Award, Eye } from 'lucide-react';
...
const AdminTeamDetail = () => {
    ...
    const fetchTeam = async () => {
        try {
            ...
            const { data } = await api.get(`/api/admin/teams/${id}`, config);
            setTeam(data);
        } catch (error) {
    ...
    useEffect(() => {
        fetchTeam();
        const intervalId = setInterval(fetchTeam, 30000);
    ...
    const handleGradeSubmit = async (reportId, projectId) => {
        ...
        try {
            const config = { headers: { Authorization: `Bearer ${user.token}` } };
            await api.put(`/api/admin/reports/${reportId}/grade`, { ...gradeData, project_id: projectId }, config);
    ...
    const handleProjectRating = async (projectId, hackathonId) => {
        try {
            const config = { headers: { Authorization: `Bearer ${user.token}` } };
            await api.put(`/api/admin/projects/${projectId}/rate`, { rating: projectRating, hackathon_id: hackathonId }, config);
    ...
    const handleDeleteArtifact = async (reportId, projectId) => {
        if (!window.confirm('Are you sure you want to delete this artifact? This action cannot be undone.')) return;
        try {
            const config = { headers: { Authorization: `Bearer ${user.token}` } };
            await api.delete(`/api/admin/reports/${reportId}?project_id=${projectId}`, config);
    ...
    const handleDownload = async (reportId, projectId, action = 'download') => {
        try {
            const config = { headers: { Authorization: `Bearer ${user.token}` } };
            const { data } = await api.get(`/api/admin/reports/${reportId}/download?project_id=${projectId}&action=${action}`, config);
            window.open(data.downloadUrl, '_blank');
        } catch (error) {
            console.error("error during secure download", error);
            alert(`Failed to generate secure ${action} link.`);
        }
    };
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-10 relative z-10">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-1 space-y-6">
                        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="card p-8 border-t-4 border-t-[#3b82f6] bg-white text-slate-800 shadow-xl shadow-blue-900/5">
                            <div className="flex flex-col items-center mb-8 border-b border-slate-100 pb-8 text-center">
                                <div className="h-24 w-24 rounded-3xl overflow-hidden border border-blue-100 flex items-center justify-center shadow-md mb-4">
                                    {team.avatar
                                        ? <img src={team.avatar} alt={`${team.team_name} avatar`} className="w-full h-full object-cover" />
                                        : <div className="w-full h-full bg-gradient-to-br from-blue-50 to-indigo-50 flex items-center justify-center text-[#3b82f6] font-black text-4xl">{initials}</div>
                                    }
                                </div>
                        ...
                        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }} className="card p-6 bg-white/60 backdrop-blur-xl border border-white/80">
                            <div className="flex items-center justify-between mb-6 border-b border-slate-100 pb-3">
                                <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
                                    <Clock size={14} className="text-[#3b82f6]" /> Entity Logs
                                </h4>
                                <span className="text-[9px] font-black text-[#3b82f6] bg-[#3b82f6]/10 px-1.5 py-0.5 rounded uppercase">Real-time</span>
                            </div>

                            <div className="space-y-4 max-h-[350px] overflow-y-auto pr-1 group shadow-[inset_0_-10px_20px_-10px_rgba(0,0,0,0.05)] custom-scrollbar">
                                {!team.activities || team.activities.length === 0 ? (
                                    <div className="text-center py-10 text-slate-300 italic text-[10px] font-bold uppercase tracking-widest">No history recorded.</div>
                                ) : (
                                    team.activities.map((act, i) => (
                                        <div key={i} className="relative pl-6 pb-4 border-l border-slate-100 last:pb-0">
                                            <div className="absolute left-[-5px] top-0 w-2.5 h-2.5 rounded-full bg-white border-2 border-[#3b82f6] shadow-sm" />
                                            ...
                                            <p className="text-xs font-bold text-slate-700 leading-snug">{act.detail}</p>
                                            <div className="mt-1">
                                                <span className="text-[9px] font-black text-[#3b82f6] uppercase tracking-widest opacity-70">[{act.action}]</span>
                                            </div>
                                        </div>
                                    ))
                                )}
                    ...
                    <div className="lg:col-span-2 space-y-8">
                        ...
                        {(!team.hackathons?.length && !team.projects?.length) ? (
                            ...
                            (() => {
                                // Group projects by hackathon, show hackathons that have projects
                                const hackathonsToShow = (team.hackathons || []).filter(h => {
                                    const hasProjects = team.projects?.some(p => p.hackathon_id === h._id);
                                    return hasProjects;
                                });
                                // Find orphaned projects (not matching any known hackathon)
                                const knownHackIds = new Set((team.hackathons || []).map(h => h._id));
                                const orphanedProjects = (team.projects || []).filter(p => !knownHackIds.has(p.hackathon_id));
                                ...
                                return (
                                    <>
                                                                                        ...
                                                                                        <div key={report._id} className="bg-slate-50/50 rounded-2xl p-5 border border-slate-100">
                                                                                            <div className="flex flex-col md:flex-row justify-between md:items-center gap-4 border-b border-slate-100 pb-4 mb-4">
                                                                                                ...
                                                                                                <div className="flex items-center gap-2">
                                                                                                    <button
                                                                                                        onClick={() => handleDownload(report._id, project._id, 'view')}
                                                                                                        className="btn-secondary py-2 px-4 text-[10px] font-black uppercase tracking-widest flex items-center gap-2 shadow-sm"
                                                                                                        title="Open in Browser"
                                                                                                    >
                                                                                                        <Eye size={14} /> Open Inline
                                                                                                    </button>
                                                                                                    <button
                                                                                                        onClick={() => handleDownload(report._id, project._id, 'download')}
                                                                                                    ...
                                                                                                    <button
                                                                                                        onClick={() => handleDeleteArtifact(report._id, project._id)}
                                                                                                ...
                                                                                                {gradingId === report._id ? (
                                                                                                    <div className="space-y-4">
                                                                                                        ...
                                                                                                        <div className="flex justify-end gap-2">
                                                                                                            ...
                                                                                                            <button onClick={() => handleGradeSubmit(report._id, project._id)} disabled={savingGrade} className="btn-cyan text-[10px] py-1.5 px-6 font-bold uppercase shadow-md shadow-blue-200">
                                        ...
                                        {/* Orphaned projects (not under any known hackathon) */}
                                        {orphanedProjects.length > 0 && (
                                            <motion.div variants={itemVariants} className="card overflow-hidden bg-white border border-amber-100 shadow-xl shadow-amber-900/5 rounded-3xl">
                                                <div className="bg-gradient-to-r from-amber-50 to-white p-6 border-b border-amber-100">
                                                    <div className="flex items-center gap-3">
                                                        <div className="h-2 w-2 rounded-full bg-amber-500 animate-pulse" />
                                                        <h4 className="text-lg font-black text-slate-800 uppercase tracking-tight">Unlinked Projects</h4>
                                                    </div>
                                                    <p className="text-[9px] font-bold text-slate-400 mt-1 uppercase tracking-widest ml-5">Projects not matched to a known hackathon</p>
                                                </div>
                                                <div className="p-8 space-y-4">
                                                    {orphanedProjects.map(project => {
                                                        const pReports = project.reports || [];
                                                        return (
                                                            <div key={project._id} className="bg-white rounded-2xl p-5 border border-slate-100 hover:border-blue-200 transition-all">
                                                                <h5 className="text-base font-black text-slate-800 tracking-tight">{project.project_title}</h5>
                                                                <p className="text-sm text-slate-400 mt-1">{project.description}</p>
                                                                <div className="mt-2 flex items-center gap-2">
                                                                    <span className={`px-2 py-0.5 rounded-lg text-[9px] font-black uppercase tracking-widest border shadow-sm ${project.status === 'Completed' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-blue-50 text-blue-600 border-blue-100'}`}>{project.status}</span>
                                                                    <span className="text-[9px] text-slate-400 font-bold">Artifacts: {pReports.length}</span>
                                                                </div>
                                                            </div>
                                                        );
                                                    })}
