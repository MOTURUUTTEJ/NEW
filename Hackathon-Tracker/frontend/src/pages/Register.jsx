            ...
            >
                <div className="flex justify-center text-[#3b82f6]">
                    <div className="relative liquid-drop p-6 bg-white/60 border border-white/80">
                        <Code2 size={48} className="relative z-10 opacity-90" strokeWidth={2.5} aria-hidden="true" />
                    </div>
                </div>
                    ...
                    <form className="space-y-6" onSubmit={handleSubmit}>
                        ...
                        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                            <div className="space-y-1">
                                ...
                                <input id="teamName" type="text" name="team_name" required className="input-field" value={teamName} onChange={(e) => setTeamName(e.target.value)} placeholder={isAdmin ? "Superuser Name" : "Awesome Coders"} autoComplete="name" />
                            </div>
                            <div className="space-y-1">
                                <label htmlFor="email" className="block text-sm font-medium text-slate-500 mb-1">Email Address</label>
                                <input id="email" type="email" name="email" required className="input-field" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="admin@example.com" autoComplete="email" spellCheck={false} />
                            </div>
                        </div>
                        ...
                        <div className="space-y-1">
                            ...
                            <div className="relative">
                                <input
                                    id="password" type={showPassword ? "text" : "password"} name="password" required className="input-field"
                                    value={password} onChange={(e) => setPassword(e.target.value)}
                                    placeholder="••••••••"
                                    autoComplete="new-password"
                                />
                                <button type="button" onClick={() => setShowPassword(!showPassword)} aria-label={showPassword ? 'Hide password' : 'Show password'} className="absolute right-4 top-4 text-slate-400 hover:text-[#3b82f6]">
                        ...
                        <button type="submit" disabled={loading} className="btn-primary w-full flex justify-center py-3 text-sm mt-8">
                            {loading ? 'Setting up environment…' : <span className="flex items-center">{isAdmin ? 'Setup Admin Profile' : 'Create Team Profile'} <ArrowRight size={16} className="ml-2" aria-hidden="true" /></span>}
                        </button>
                    </form>
                </div>
            </motion.div>
