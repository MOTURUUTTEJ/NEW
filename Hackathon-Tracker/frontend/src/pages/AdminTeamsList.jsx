...
import { Search, Filter, ChevronRight, Code2, LogOut, Database, Trash2, Download, Users, Globe, TrendingUp } from 'lucide-react';
...
const AdminTeamsList = () => {
    ...
    const { teams: contextTeams, fetchTeams, dashboardData } = useContext(AdminContext);
    const [teams, setTeams] = useState(contextTeams || []);
    const [loading, setLoading] = useState(!contextTeams?.length);
    ...
    useEffect(() => {
        if (contextTeams) {
            ...
            if (filterStatus !== 'All' && dashboardData?.projects) {
                filtered = filtered.filter(t => {
                    const teamProjects = dashboardData.projects.filter(p => p.team_id === t._id);
                    return teamProjects.some(p => p.status === filterStatus);
                });
            }
    ...
    }, [contextTeams, searchTerm, filterStatus, sortBy, dashboardData]);

    useEffect(() => {
        fetchTeams();
    }, [fetchTeams, searchTerm, filterStatus]); // Fetch when filters change
        ...
        <div className="min-h-screen pb-16 relative overflow-hidden bg-gradient-to-br from-slate-50 via-blue-50/40 to-indigo-50/60">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-10 relative z-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
                    ...
                    <div className="bg-white/40 backdrop-blur rounded-2xl border border-white/80 flex items-center justify-center text-xs font-bold text-slate-400 uppercase tracking-widest px-4">
                                        ...
                                        <tr key={team._id} className="group hover:bg-blue-50/30 transition-all duration-300">
                                            <td className="px-8 py-5 whitespace-nowrap">
                                                <div className="flex items-center">
                                                    <div className="flex-shrink-0 h-12 w-12 rounded-2xl overflow-hidden border border-blue-100 shadow-sm group-hover:shadow-md group-hover:scale-105 transition-all">
                                                        {team.avatar
                                                            ? <img src={team.avatar} alt={`${team.team_name} avatar`} className="w-full h-full object-cover" />
                                                            : <div className="w-full h-full bg-gradient-to-br from-blue-50 to-indigo-50 flex items-center justify-center text-[#3b82f6] font-black text-lg">
